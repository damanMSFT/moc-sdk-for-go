...
// CreateOrUpdate
func (c *client) Upload(ctx context.Context, group, container, name string, vhd *storage.VirtualHardDisk) (*storage.VirtualHardDisk, error) {
	request, err := getVirtualHardDiskOperationRequest(wssdcloudcommon.ProviderAccessOperation_VirtualHardDisk_Upload, group, container, name, vhd)
	if err != nil {
		return nil, err
	}
	...
	vhds := getVirtualHardDisksFromOperationResponse(response, group)

	if len(*vhds) == 0 {
		return nil, fmt.Errorf("[VirtualHardDisk][Upload] Unexpected error: Did not return any result from the Upload operation")
	}
...
func getVirtualHardDiskOperationRequest(opType wssdcloudcommon.ProviderAccessOperation, group, container, name string, storage *storage.VirtualHardDisk) (*wssdcloudstorage.VirtualHardDiskRequest, error) {
	request := &wssdcloudstorage.getVirtualHardDiskOperationRequest{
		OperationType:    opType,
		VirtualHardDisks: []*wssdcloudstorage.VirtualHardDisk{},
	}
...
func getVirtualHardDisksFromOperationResponse(response *wssdcloudstorage.VirtualHardDiskOperationResponse, group string) *[]storage.VirtualHardDisk {
	virtualHardDisks := []storage.VirtualHardDisk{}
	for _, vhd := range response.GetVirtualHardDisks() {
		virtualHardDisks = append(virtualHardDisks, *(getVirtualHardDisk(vhd, group)))
	}
